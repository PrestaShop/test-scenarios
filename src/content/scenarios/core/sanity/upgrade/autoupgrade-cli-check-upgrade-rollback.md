...
| *CLI Upgrade*<br><br>Launch the command "php bin/console update:start " | Your terminal should say " Not enough arguments (missing: "admin-dir")." |
| Launch the command "php bin/console update:start --xml=prestashop_9.0.0.xml admin-dev" | Your terminal should say "ERROR - Both 'xml' and 'zip' files attributes must be provided to use the local channel." |
| Launch the command "php bin/console update:start --zip=prestashop_9.0.0.zip admin-dev" | Your terminal should say "ERROR - Both 'xml' and 'zip' files attributes must be provided to use the local channel." |
| Launch the command "php bin/console update:start --xml=pr2.xml --zip=pr2.zip admin-dev" | Your terminal should say "ERROR - File pr2.zip does not exist. Unable to select that channel." |
| {color:#FF0000}*POUIIIIIIIKKKK*{color} |  |
...
| * Launch the command "php bin/console update:start  admin-dev" | * The terminal should start with<br>" INFO - Starting update...<br>INFO - Destination version: {test-param}Last_version_8{test-param} "<br>and end with : <br>"INFO - The database has been cleaned.<br>INFO - Shop updated to {test-param}Last_version_8{test-param} . Congratulations! You can now reactivate your shop." |
...
| * Launch the command "php bin/console update:start --xml=prestashop_{test-param}Last_version_9{test-param} .xml --zip=prestashop_{test-param}Last_version_9{test-param} .zip admin-dev" | * The terminal should say :<br>"INFO - Update process will use archive.<br>INFO - Configuration successfully updated.<br>INFO - Starting update...<br>INFO - Destination version: {test-param}Last_version_9{test-param} " |
| * Launch the command "php bin/console backup:restore --backup=1.7.7.8_ID admin-dev" | * Your terminal should say at start "INFO - Restoring files ..." and end with "INFO - Restoration process done. Congratulations! You can now reactivate your store." |
